...
if (strcmp(arbConfig.model, 'M8195A_2ch') || strcmp(arbConfig.model, 'M8195A_2ch_mrk') || strcmp(arbConfig.model, 'M8195A_2ch_dupl') || strcmp(arbConfig.model, 'M8195A_4ch'))
    mpulse_demo2();
...
if (segmNum == 1)
    ...
    xfprintf(f, '*CLS');  % clear the error caused by deleting segments that don't exist
end


function gen_arb_M8195A(f, chan, wfm, segmNum)
segmentLength = length(wfm);
xfprintf(f, sprintf(':TRACe%d:DELete %d', chan, segmNum));
xfprintf(f, sprintf(':TRACe%d:DEFine %d,%d', chan, segmNum, segmentLength));
