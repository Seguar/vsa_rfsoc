...
% try to find a previously stored VISA adddress of the U9391
try
    arbCfgFile = iqarbConfigFilename();
    acs = load(arbCfgFile);
    set(handles.editVisaAddr, 'String', acs.arbConfig.visaAddrU9391);
catch
end
...
id = testRcalConnection(visaAddr);
if (~isempty(id))
    set(handles.pushbuttonTest, 'Background', 'green');
    saveVisaAddr(visaAddr);
else
...
function saveVisaAddr(visaAddr)
try
    arbCfgFile = iqarbConfigFilename();
    acs = load(arbCfgFile);
    acs.arbConfig.visaAddrU9391 = visaAddr;
    acs.arbConfig.isU9391connected = true;
    save(arbCfgFile, '-struct', 'acs');
catch
end
