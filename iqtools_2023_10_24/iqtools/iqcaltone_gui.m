...
f = iqopen();
if (~isempty(f))
    minAmpl = str2double(xquery(f, ':VOLT? MIN'));
    maxAmpl = str2double(xquery(f, ':VOLT? MAX'));
...
freq = -1;
try
    freq = iqparse(get(handles.editFreq, 'String'), 'scalar');
catch ex
    msgbox(ex.message);
end
...
power = -1;
try
    power = iqparse(get(handles.editPower, 'String'), 'scalar');
catch ex
    msgbox(ex.message);
end
...
numFreq = -1;
try
    numFreq = iqparse(get(handles.editNumTones, 'String'), 'scalar');
catch ex
    msgbox(ex.message);
    result = 0;
end
...
numAmpl = iqparse(get(handles.editNumAmpl, 'String'), 'vector');
startAmpl = iqparse(get(handles.editStartAmpl, 'String'), 'vector');
stopAmpl = iqparse(get(handles.editStopAmpl, 'String'), 'vector');
if (numAmpl > 1)
...
phase = [];
try
    phase = iqparse(get(handles.editPhase, 'String'), 'scalar');
catch ex
    msgbox(ex.message);
end
