...
%fileName = 'CRPAT.ptrn';
if (nargin == 0)
    [filename, pathname] = uigetfile({'*.ptrn;*.txt;*.csv'},'Select a *.ptrn file');
    if filename ~= 0
...
if ~badfile
    ...
    if ~badfile         % don't go further if there is some error in format
        if ~txtfile
            ...
            fclose(fid);
            % if values exceed the range [0...1], scale them accordingly
            maxV = max(fileCharData);
            minV = min(fileCharData);
            if (minV < 0 || maxV > 1)
                if (minV ~= maxV)
                    fileCharData = (fileCharData - minV) ./ (maxV - minV);
                else
                    fileCharData = (fileCharData - minV);
                end
            end
        end     %%% end of txt file
