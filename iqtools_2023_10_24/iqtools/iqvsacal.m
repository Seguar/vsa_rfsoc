...
result = vsaCal(symbolRate, modType, filterType, filterBeta, carrierOffset, fc, filterLength, convergence, resultLength, recalibrate, useHW, doCal);
end


function result = vsaCal(symbolRate, modType, filterType, filterBeta, carrierOffset, fc, filterLength, convergence, resultLength, recalibrate, useHW, doCal)
    result = -1;
    vsaApp = vsafunc([], 'open');
    if (~isempty(vsaApp))
