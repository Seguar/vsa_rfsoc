...
try
    ...
    fc = iqparse(get(handles.editCenterFrequency, 'String'), 'scalar');
    amplCutoff = iqparse(get(handles.editAmplCutoff, 'String'), 'scalar');
    correction = get(handles.checkboxCorrection, 'Value');
    showDropped = get(handles.checkboxShowDropped, 'Value');
    maxsamples = iqparse(get(handles.editMaxSamples, 'String'), 'scalar');
    normalize = get(handles.checkboxNormalize, 'Value');
    segmentNum = iqparse(get(handles.editSegmentNumber, 'String'), 'scalar');
catch ex
...
if (strcmp(fct, 'code'))
    ...
    for i=1:length(pulseTable)
        ...
        code = sprintf('%spulseTable(%d).riseTime = [%s];\n', code, i, iqengprintf(pulseTable(i).riseTime));
        code = sprintf('%spulseTable(%d).fallTime = [%s];\n', code, i, iqengprintf(pulseTable(i).fallTime));
        code = sprintf('%spulseTable(%d).offset = [%s];\n', code, i, iqengprintf(pulseTable(i).offset));
        code = sprintf('%spulseTable(%d).modulationType = [%s];\n', code, i, iqengprintf(pulseTable(i).modulationType));
        code = sprintf('%spulseTable(%d).span = [%s];\n', code, i, iqengprintf(pulseTable(i).span));
        code = sprintf('%spulseTable(%d).amplitude = [%s];\n', code, i, iqengprintf(pulseTable(i).amplitude));
    ...
    for i=1:length(toneTable)
        ...
        code = sprintf('%stoneTable(%d).amplitude = [%s];\n', code, i, iqengprintf(toneTable(i).amplitude));
...
try
    ...
    {'delay', 'PRI', 'PW', 'tt', 'offset', 'span', 'amplitude', ...
...
for i=1:length(pulseTable)
    ...
    pulseTable(i).delay  = iqparse(pulseTable(i).delay, 'vector');
    pulseTable(i).PRI    = iqparse(pulseTable(i).PRI, 'vector');
    pulseTable(i).PW     = iqparse(pulseTable(i).PW, 'vector');
    pulseTable(i).riseTime = iqparse(pulseTable(i).tt, 'vector');
    pulseTable(i).fallTime = iqparse(pulseTable(i).tt, 'vector');
    pulseTable(i).offset = iqparse(pulseTable(i).offset, 'vector');
    pulseTable(i).modulationType = 'Increasing';        % t.b.d.: include in table
    pulseTable(i).span   = iqparse(pulseTable(i).span, 'vector');
    pulseTable(i).amplitude  = iqparse(pulseTable(i).amplitude, 'vector');
    pulseTable(i).scanPeriod = iqparse(pulseTable(i).scanPeriod, 'vector');
    pulseTable(i).scanAz = iqparse(pulseTable(i).scanAz, 'vector');
    pulseTable(i).scanSq = iqparse(pulseTable(i).scanSq, 'vector');
    numPulse(i) = max([length(pulseTable(i).delay) ...
                       length(pulseTable(i).PRI) ...
                       length(pulseTable(i).PW) ...
                       length(pulseTable(i).riseTime) ...
                       length(pulseTable(i).fallTime) ...
                       length(pulseTable(i).span) ...
                       length(pulseTable(i).offset) ...
                       length(pulseTable(i).amplitude)]);
    ...
    pulseTable(i).riseTime = fixlength(pulseTable(i).riseTime, numPulse(i));
    pulseTable(i).fallTime = fixlength(pulseTable(i).fallTime, numPulse(i));
    pulseTable(i).span     = fixlength(pulseTable(i).span, numPulse(i));
    pulseTable(i).offset   = fixlength(pulseTable(i).offset, numPulse(i));
    pulseTable(i).amplitude= fixlength(pulseTable(i).amplitude, numPulse(i));
    for k = 1:numPulse(i)
        if (sum(pulseTable(i).delay + pulseTable(i).PW(k) + pulseTable(i).riseTime(k) + pulseTable(i).fallTime(k)) > pulseTable(i).PRI(k))
            errordlg(sprintf('Line %d, Pulse #%d: Delay + Pulse Width + Rise Time + Fall Time > PRI', i, k));
            err = 1;
            break;
        end
    end
...
try
    ...
    {'offset', 'amplitude', 'fmfreq', 'fmdev', 'amfreq', 'amdepth'}, 2);
    for i=1:length(toneTable)
        toneTable(i).offset  = iqparse(toneTable(i).offset, 'vector');
        if (~isvector(toneTable(i).offset)); error('frequency offset must be a vector of values'); end
        toneTable(i).amplitude = iqparse(toneTable(i).amplitude, 'vector');
        if (~isvector(toneTable(i).amplitude)); error('magnitude must be a vector of values'); end
        toneTable(i).fmfreq  = iqparse(toneTable(i).fmfreq, 'vector');
        if (~isscalar(toneTable(i).fmfreq)); error('FM frequency must be a scalar value'); end
        toneTable(i).fmdev   = iqparse(toneTable(i).fmdev, 'vector');
        if (~isscalar(toneTable(i).fmdev)); error('FM deviation must be a scalar value');  end
        toneTable(i).amfreq  = iqparse(toneTable(i).amfreq, 'vector');
        if (~isscalar(toneTable(i).amfreq)); error('AM frequency must be a scalar value');  end
        toneTable(i).amdepth = iqparse(toneTable(i).amdepth, 'vector');
...
value = -1;
try
    value = iqparse(get(handles.editMaxSamples, 'String'), 'scalar');
catch ex
    msgbox(ex.message);
    result = 0;
end
...
value = -1;
try
    value = iqparse(get(handles.editMaxTime, 'String'), 'scalar');
catch ex
    msgbox(ex.message);
    result = 0;
end
...
value = [];
try
    value = iqparse(get(handles.editAmplCutoff, 'String'), 'scalar');
catch ex
    msgbox(ex.message);
    result = 0;
end
...
try
    samplerate = iqparse(get(handles.editSampleRate, 'String'), 'scalar');
    val = iqparse(get(handles.editMaxSamples, 'String'), 'scalar');
...
try
    samplerate = iqparse(get(handles.editSampleRate, 'String'), 'scalar');
    val = iqparse(get(handles.editMaxTime, 'String'), 'scalar');
