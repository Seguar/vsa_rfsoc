...
tilt = iqparse(get(handles.editTilt, 'String'), 'scalar');
...
for i = 1:length(freqTable)
    freqTable(i).frequency = 1e6 * iqparse(freqTable(i).frequency, 'vector');
    freqTable(i).bandwidth = 1e6 * iqparse(freqTable(i).bandwidth, 'vector');
    freqTable(i).power = iqparse(freqTable(i).power, 'vector');
end
...
%    set(handles.textPlaceHolder1, 'Visible', 'off');
%    set(handles.textPlaceHolder2, 'Visible', 'off');
    segment = iqparse(get(handles.editSegment, 'String'), 'scalar');
...
if (get(handles.checkboxAmplitudeAuto, 'Value'))
    ...
    scaleManual = iqparse(get(handles.editAmplitudeScale, 'String'), 'scalar');
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
carrierNum = iqparse(get(handles.editVSACarrierNum, 'String'), 'scalar');
freqTable = getTable(handles);
found = 0;
for i = 1:length(freqTable)
    freq = 1e6 * iqparse(freqTable(i).frequency, 'vector');
    symbolRate = 1e6 * iqparse(freqTable(i).bandwidth, 'vector');
...
if (found)
    ...
    filterIdx = get(handles.popupmenuFilterType, 'Value');
    filterBeta = iqparse(get(handles.editFilterBeta, 'String'), 'scalar');
    resultLength = iqparse(get(handles.editVSAResultLength, 'String'), 'scalar');
    fc = freq(carrierNum);
    filterLength = 99;
    convergence = 1e-8;
    vsaApp = vsafunc([], 'open');
...
for i = 1:len
    fx = freqTable(i);
    freq = iqparse(fx.frequency, 'vector');
    if (length(freq) > 1)
...
for i = 1:length(freqTable)
    try
        freq = iqparse(freqTable(i).frequency, 'vector');
        len = length(freq);
        if (len > 1)
    ...
    try
        bw = iqparse(freqTable(i).bandwidth, 'vector');
    catch ex
        errordlg(sprintf('invalid bandwidth in line %d: %s', i, ex.message));
    end
    try
        pwr = iqparse(freqTable(i).power, 'vector');
    catch ex
...
try
    f = iqopen();
    xfprintf(f, sprintf(':stab:dyn:sel %d', val - 1));
    iqclose(f);
catch ex
    errordlg(ex.message);
end
