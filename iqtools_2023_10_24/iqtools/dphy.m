...
if (doSetup)
    ...
    model = xquery(fscope, '*idn?');
    isMXR = ~isempty(strfind(model, 'MXR'));
    if (dParam.hsDataRate <= 2e9 && ~isMXR)
        xfprintf(fscope, sprintf(':acquire:bandwidth 8 GHz'));
    elseif (dParam.hsDataRate <= 3e9 && ~isMXR)
...
if (~isempty(syncCfg))
    ...
    xfprintf(fsync, ':abor');
    xfprintf(fsync, ':inst:mmod:conf 1');
    configMode = xquery(fsync, ':INSTrument:MMODule:CONF?');
...
if (~isempty(strfind(awg1.model, 'M8190A')))
    iqseq('delete', [], 'arbConfig', awg1, 'chMap', [1 0; 0 1], 'keepOpen', 1);
end
...
%     iqdownload(sumA(:,2), fs, 'arbConfig', awg1, 'channelMapping', [0 0; 1 0; 0 0; 0 0], 'run', 0);
    ...
    for i=1:size(wfms,1)
        iqdownload([wfms{i,1} wfms{i,2}], fs, 'arbConfig', awg1, 'channelMapping', dParam.chMap([1 2],[1 3 5 7]), 'keepOpen', 1, 'run', 0, 'segmentNumber', i+1);
...
% delete all segments
if (~isempty(awg2))
    iqseq('delete', [], 'arbConfig', awg2, 'chMap', [1 0; 0 1], 'keepOpen', 1, 'run', 0);
end
...
elseif (~isempty(awg2))
    ...
    for i=1:size(wfms,1)
        iqdownload([wfms{i,1} wfms{i,2}], fs, 'arbConfig', awg2, 'channelMapping', dParam.chMap([3 4],[1 3 5 7]), 'keepOpen', 1, 'run', 0, 'segmentNumber', i+1);
...
if (~isempty(strfind(awg1.model, 'M8190A')))
    ...
    seq(i).sequenceEnd = 1;
    seq(i).scenarioEnd = 1;
    iqseq('define', seq, 'arbConfig', awg1, 'chMap', [1 0; 0 1], 'keepOpen', 1, 'run', 0);
    if (~isempty(awg2))
        iqseq('define', seq, 'arbConfig', awg2, 'chMap', [1 0; 0 1], 'keepOpen', 1, 'run', 0);
    end

    % set AWG #1 to triggered or continuous - depending on sync module
    iqseq('triggerMode',  ~isempty(syncCfg), 'arbConfig', awg1, 'chMap', [1 0; 0 1], 'keepopen', 1);
    % turn on triggered mode in AWG #2 in any case
    if (~isempty(awg2))
        iqseq('triggerMode', 'triggered', 'arbConfig', awg2, 'chMap', [1 0; 0 1], 'keepopen', 1);
    end
    ...
    if (~isempty(awg2))
        ...
        iqseq('mode', 'STSC', 'arbConfig', awg2, 'chMap', [1 0; 0 1], 'keepopen', 1);
    ...
    iqseq('mode', 'STSC', 'arbConfig', awg1, 'chMap', [1 0; 0 1], 'keepopen', 1);
...
if (strcmp(cmd, 'init'))
    ...
    if (~isempty(p1))
        ...
        wfms{idx,1} = complex(real(sig(:,1)), real(sig(:,2)));
        wfms{idx,2} = complex(real(sig(:,3)), real(sig(:,4)));
        idx = idx + 1;
    end
    if (~isempty(dParam.lpPattern))
...
meas_low = -1*ones(1,4);
for i = 1:numCh
    meas_high(i) = str2double(xquery(fscope, sprintf(':meas:vtop? chan%d', i)));
    meas_low(i) = str2double(xquery(fscope, sprintf(':meas:vbase? chan%d', i)));
end
...
maxAmpl = str2double(xquery(f1, ':VOLT:AMPL? MAX'));
...
model = xquery(fscope, '*idn?');
% find out if it is an MXR. Set MXR to 50 Ohm. Factory default is 1 MOhm
if (~isempty(strfind(model, 'MXR')))
    isMXR = 1;
    scopeMeasReadyCmd = 'pder?';
else
    isMXR = 0;
    scopeMeasReadyCmd = 'ader?';
end
...
for i = 1:4
    ...
    if (isMXR)
        xfprintf(fscope, sprintf(':chan%d:inp DC50', i));
    end
end
trigLev = offs;
...
xfprintf(fscope, ':run');
res = xquery(fscope, scopeMeasReadyCmd);
if (eval(res) ~= 1)
    % try one more time
    res = xquery(fscope, scopeMeasReadyCmd);
    if (eval(res) ~= 1)
...
model = xquery(fscope, '*idn?');
% find out if it is an MXR. need to use a different command to check if
...
while (doMeasAgain)
    ...
    xfprintf(fscope, sprintf(':meas:stat on'));
    pause(measDelay);
    xquery(fscope, scopeMeasReadyCmd);
