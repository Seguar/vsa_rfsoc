...
if nargin-3 >= 1
    switch varargin{1}
        case 'text'
            set(handles.editText, 'String', cellstr(varargin{2}));
        case 'file'
            f = fopen(varargin{2}, 'rt');
            if (f >= 0)
                text = fread(f, inf, '*char')';
                set(handles.editText, 'String', cellstr(text));
                fclose(f);
            else
                errordlg(sprintf('cannot open %s', varargin{2}));
            end
    end
end
