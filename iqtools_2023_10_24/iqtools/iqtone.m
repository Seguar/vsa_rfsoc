...
% Generate an IQ or RF multitone waveform
%
% Parameters are passed as property/value pairs. Properties are:
% 'sampleRate' - sample rate in Hz (if omitted, use default sample rate)
...
% 'phase' - vector of phases in radians or 'Random', 'Zero', 'Increasing',
%          'Parabolic'
...
% 'channelMapping' - channel mapping as defined in iqdownload.m
%
% iqtone returns the following values:
%  signal - a vector or array with with the resulting waveform(s)
%  tone - list of tone frequencies
%  magnitude - list of magnitude values (in dB)
%  phase - list of phase values (in radians)
%  channelMapping - the channel mapping that belongs to the signal
%              the format of chMap is described in iqdownload.m
%
...
while (i <= nargin)
    if (ischar(varargin{i}))
        switch lower(varargin{i})
            ...
            case {'chmap', 'channelmapping'}; channelMapping = varargin{i+1};
...
% apply correction
if (correction)
    [iqdata, channelMapping] = iqcorrection(iqdata, sampleRate, 'chMap', channelMapping, 'normalize', normalize, 'arbConfig', arbConfig);
end
% normalize
if (normalize)
