...
set(handles.popupmenuIScope, 'String', {'1A', '1B', 'DIFF1A', '1C', '1D', 'DIFF1C', '2A', '2B', 'DIFF2A', '2C', '2D', 'DIFF2C', '3A', '3B', 'DIFF3A', '3C', '3D', 'DIFF3C', '4A', '4B', 'DIFF4A', '4C', '4D', 'DIFF4C', 'FUNC1', 'FUNC2', 'FUNC3', 'FUNC4', 'FUNC5', 'FUNC6', 'FUNC7', 'FUNC8', 'FUNC9'});
...
try
    ...
    if (get(handles.popupmenuIAWG, 'Value') > length(ch1List))
        set(handles.popupmenuIAWG, 'String', ch1List);
    end
    set(handles.popupmenuIAWG, 'String', ch1List);
    ...
    elseif (strcmp(arbConfig.model, 'M8195A_2ch') || strcmp(arbConfig.model, 'M8195A_2ch_mrk') || strcmp(arbConfig.model, 'M8195A_2ch_dupl'))
    ...
    elseif (strcmp(arbConfig.model, 'M8199A_ILV'))
        set(handles.popupmenuTrigAWG, 'String', {'Marker'});
        set(handles.popupmenuTrigAWG, 'Value', 1);
        set(handles.editBandwidth, 'String', iqengprintf(70e9));
        set(handles.editBandwidth, 'Enable', 'on');
        set(handles.textBandwidth, 'Enable', 'on');
        set(handles.editMaxFreq, 'String', iqengprintf(70e9));
        % set not-yet implemented features to inactive
        set(handles.editAnalysisAverages, 'String', '1');  
        set(handles.editAnalysisAverages, 'Enable', 'off');
        set(handles.text10, 'Enable', 'off');
        
        set(handles.checkboxScopeRST, 'Value', 0); 
        set(handles.checkboxScopeRST, 'Enable', 'off'); 
        set(handles.text15, 'Enable', 'off');
        
        set(handles.checkboxAWGRST, 'Value', 0); 
        set(handles.checkboxAWGRST, 'Enable', 'off');
        set(handles.text16, 'Enable', 'off');
    elseif (strcmp(arbConfig.model, 'M8199A'))
        set(handles.popupmenuTrigAWG, 'String', {'Marker'});
        set(handles.popupmenuTrigAWG, 'Value', 1);
        set(handles.editBandwidth, 'String', iqengprintf(64e9));
        set(handles.textBandwidth, 'Enable', 'on');
        set(handles.editBandwidth, 'Enable', 'on');
        set(handles.editMaxFreq, 'String', iqengprintf(64e9));
...
idxChan =get(handles.popupmenuIScope, 'Value') ; 
ChanList =get(handles.popupmenuIScope, 'String') ;
ChanSelected = ChanList{idxChan};

% if ~isempty(strfind(ChanSelected, 'UXR'))
%    set(handles.editSampleRate, 'String', '256e9');
...
valMaxFreq = [];
try
    valBW = iqparse(get(handles.editBandwidth, 'String'), 'vector');
    valMaxFreq = iqparse(get(handles.editMaxFreq, 'String'), 'vector');
catch ex
end
...
%         tones = (1:numTones)/numTones * maxFreq;
    ...
    bandwidth = iqparse(get(handles.editBandwidth, 'String'), 'vector');
    nPer = iqparse(get(handles.editNumPer, 'String'), 'vector');
    ...
    elseif (get(handles.radiobuttonSA, 'Value'))
        analyzer = 'SA';
    elseif (get(handles.radiobuttonRTS, 'Value'))
        analyzer = 'RTS';
    else
...
set(handles.pushbuttoSaveAs, 'Enable', 'off');
try
...
set(handles.pushbuttoSaveAs, 'Enable', 'on');

try hMsgBox.delete(); catch; end
...
% --- Executes on button press in radiobuttonRTS.
function radiobuttonRTS_Callback(hObject, eventdata, handles)
% hObject    handle to radiobuttonDCA_IEEE (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
setAnalyzer(handles, 'RTS');


function setAnalyzer(handles, analyzer)
chListDCA = {'1A', '1B', 'DIFF1A', '1C', '1D', 'DIFF1C', '2A', '2B', 'DIFF2A', '2C', '2D', 'DIFF2C', '3A', '3B', 'DIFF3A', '3C', '3D', 'DIFF3C', '4A', '4B', 'DIFF4A', '4C', '4D', 'DIFF4C', 'FUNC1', 'FUNC2', 'FUNC3', 'FUNC4', 'FUNC5', 'FUNC6', 'FUNC7', 'FUNC8', 'FUNC9'};
chListRTS = {'1', '2', '3', '4', 'DIFF1-2', 'DIFF1-3', 'DIFF2-4', 'DIFF3-4', 'FUNC1', 'FUNC2', 'FUNC3', 'FUNC4', 'FUNC5', 'FUNC6', 'FUNC7', 'FUNC8', 'FUNC9'};
enableTrigger = 'on';
switch(analyzer)
    case 'DCA_IEEE'
        x = 'on';
        y = 'on';
        z = 'off';
        tmp = get(handles.popupmenuIScope, 'String')';
        if (~isequal(tmp, chListDCA))
            set(handles.popupmenuIScope, 'String', chListDCA);
            set(handles.popupmenuIScope, 'Value', 1);
        end
    case 'DCA'
        x = 'on';
        y = 'on';
    ...
    case 'RTS'
        ...
        z = 'off';
        tmp = get(handles.popupmenuIScope, 'String')';
        if (~isequal(tmp, chListRTS))
            set(handles.popupmenuIScope, 'String', chListRTS);
            set(handles.popupmenuIScope, 'Value', 1);
        end
end
set(handles.textTrigger, 'Enable', enableTrigger);
set(handles.textScopeChan, 'Enable', x);
set(handles.popupmenuTrigAWG, 'Enable', enableTrigger);
set(handles.popupmenuTrigScope, 'Enable', enableTrigger);
