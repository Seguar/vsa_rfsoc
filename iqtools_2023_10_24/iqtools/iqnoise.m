...
while (i <= nargin)
    if (ischar(varargin{i}))
        switch lower(varargin{i})
            ...
            case 'compression';compression = varargin{i+1};
...
% apply compression (before or after linear compensation??)
if (compression)
%    if (isreal(iqdata))
        iqdata = iqdata .* (compression * abs(iqdata) + (1-compression));
%     else
%         iqdata = complex(...
%             real(iqdata) .* (compression * abs(real(iqdata)) + (1-compression)), ...
%             imag(iqdata) .* (compression * abs(imag(iqdata)) + (1-compression)));
%     end
end
% apply correction
if (correction)
