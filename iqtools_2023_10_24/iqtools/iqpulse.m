...
%     iqdata = iqparse(customIQPulse, 'vector');
...
% subsequent pulses - but this is not possible here
    ...
    if (min(offTime) < 0)  % Wychock: Double handling
        ...
        if (min(offTime) < -1.0e-14) && (pri(1) ~= 0)
        ...
        pri(find(offTime < 0)) = tmp(find(offTime < 0));
    end
    ...
    if (modval ~= 0)
        ...
        if exactpri && lcm(numSamples, arbConfig.segmentGranularity) < arbConfig.maximumSegmentSize/2
            ...
            pri(end) = pri(end) + (corr / sampleRate);
...
% calculate the pulse envelope
    ...
    for i = 1:length(pri)
        ...
        pidx = (ih(2):ih(3)-1);
        fidx = (ih(3):ih(4)-1);
        % include rise & fall in envelope
        envelopeStarts(i) = ih(1);
        envelopeStops(i) = ih(4);
        ...
        if (~isempty(rr))
            envelope(ridx) = linamp .* rise_wave;
        end
        envelope(pidx) = linamp;
        
        if (riseTime(i) == 0 && ~isempty(pidx) && pidx(1) > 0)
            envelope(pidx(1)) = linamp;
        end
        
        if (~isempty(fr))
            envelope(fidx) = linamp .* fall_wave;
        end
...
% calculate the phase based on span and offset
    ...
    for i = 1:length(pri)
        ...
        switch lower(chirpType{i})
            ...
            case 'custom phase code'
                x = pr;
                try
                    tmp = pm_fct(x,i);                
                    tmp = repmat(tmp, ceil(length(pr) / length(tmp)), 1);
                    pm_on = tmp(1:length(pr));
                catch ex
                    errordlg(ex.message);
                end
            otherwise
                error('undefined modulation type');
        end
...
function iqdata = calcCustomIQ(numSamples, sampleRate, pri, customIQPulse, envelope, offset, phase, envelopeStarts, envelopeStops)
    ...
    assignin('base', 'sampleRate', sampleRate);         % allow the formula use current sampleRate
    iqdataCalc = iqparse(customIQPulse, 'vector');
