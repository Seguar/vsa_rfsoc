...
% THIS SOFTWARE MAY ONLY BE USED IN CONJUNCTION WITH KEYSIGHT INSTRUMENTS. 
    ...
    if (fs > 0)
        ...
        for col = 1:min(size(channelMapping, 2) / 2, size(data,2))
            for ch = find(channelMapping(:, 2*col-1))'
                gen_arb_M8195A(arbConfig, f, ch, real(data(:,col)), marker1, segmNum, run, fs);
            end
            for ch = find(channelMapping(:, 2*col))'
                gen_arb_M8195A(arbConfig, f, ch, imag(data(:,col)), marker2, segmNum, run, fs);
            end
        end
    end
%     % direct mode waveform download
...
%        end
    ...
    if (isfield(arbConfig,'amplitude'))
        if (size(arbConfig.amplitude, 2) < 4)
            arbConfig.amplitude = repmat(arbConfig.amplitude, 1, 4);
        end
    ...
    if (isfield(arbConfig,'offset'))
        if (size(arbConfig.offset, 2) < 4)
            arbConfig.offset = repmat(arbConfig.offset, 1, 4);
        end
