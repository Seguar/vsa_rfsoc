...
try
    if (strfind(fileType, 'ASCII'))
        try
            ...
            try
                % try to read the whole file (no header)
                data = csvread(filename);
            catch ex
                % try to read a file with header
                f = fopen(filename, 'r');
                lineNum = 1;
                l = fgets(f);
                while (l ~= -1)
                    l = strtrim(l);
                    if (strcmp(l, 'Y') || strncmp(l, 'Y1', 2) || strncmpi(l, 'Data', 4))
                        break;
                    end
                    [~, cnt] = sscanf(l, '%g');
                    if (cnt > 0)
                        % line starts with a number, assume the data
                        % content starts (csvread uses 0-based line numbers)
                        lineNum = lineNum - 1;
                        break;
                    end
                    if (strncmpi(l, 'Samplerate', 10))
                        pos = strfind(l,'=');
                        if (isempty(pos))
                            pos = 10;
                        end
                        sampleRate = iqparse(l(pos+1:end));
                    end
                    if (strncmpi(l, 'XDelta,', 7))
                        sampleRate = 1/iqparse(l(8:end));
                    end
                    if (strncmp(l, 'XInc,', 5))
                        sampleRate = 1/iqparse(l(6:end));
                    end
                    l = fgets(f);
                    lineNum = lineNum + 1;
                end
                ...
                fclose(f);
                data = csvread(filename, lineNum, 0);
            end
            ...
            if (isempty(sampleRate))
                err = 'You must specify a sample rate or load a CSV file with header information that contains sample rate information.';
                error(err);
            end
        catch ex
    ...
    elseif (~isempty(strfind(fileType, 'MATLAB')))
        ...
        % if the file contains a single struct, then treat the struct
        % as the top-level
        if (length(fields) == 1 && isa(data.(fields{1}), 'struct'))
            data = data.(fields{1});
            fields = fieldnames(data);
        end
...
if (~isempty(windowMethod))
    ...
    if (windowFactor >= 1)
        ...
        if (windowFactor >= 1)
            errordlg('overlap range must be less than waveform length');
            return;
        end
    end
    switch (lower(windowMethod))
        ...
        case 'crossfade'
            ...
            % Note, that the waveform length is reduced by the windowsize
            windowSamples = floor(windowFactor * windowSize / 2) * 2;
            win = (cos(pi*(1:windowSamples)/windowSamples)'+1)/2;
            iqTmp = iqdata(1:end-windowSamples);
            iqTmp(1:windowSamples) = iqdata(1:windowSamples) .* (1-win) + iqdata(end-windowSamples+1:end) .* win;
            iqdata = circshift(iqTmp, -windowSamples/2);
        otherwise
            errordlg(['unknown windowMethod: ' windowMethod]);
    end
end
...
% set min/max values
if (isreal(iqdata))
    minVal = min(min(iqdata));
    maxVal = max(max(iqdata));
else
