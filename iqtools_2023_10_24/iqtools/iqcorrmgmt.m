...
function updatePerChannelCorr(handles, freq, Y, AWGChannels, prompt)
...
written = checkMergeOverwrite(perChannelCorr, AWGChannels, prompt);
if (written)
    updateAxes(handles);
end


function written = checkMergeOverwrite(perChannelCorr, AWGChannels, prompt)
...
% prompt can be:
%   'Auto' - determine automatically if the user should be prompted
%   'Ask' - prompt the user for overwrite vs. merge
%   'Overwrite' - overwrite all channels
%   'Merge' - merge current channels with previously existing channels
written = 0;
ampCorrFile = iqampCorrFilename();
clear acs;
oldPcc = [];
...
if (strcmpi(prompt, 'Auto') && ...
    ...
    % partial overlap and not 'Auto' prompt
    if (strcmpi(prompt, 'Overwrite') || strcmpi(prompt, 'Merge'))
        res = prompt;
    else
...
try
    ...
    if (~showPhase)
        ...
        if (isfield(acs, 'AWGChannels') && ~isempty(acs.AWGChannels))
            leg = {};
            for k = 1:length(acs.AWGChannels)
                leg{k} = sprintf('Magn./Ch%d', acs.AWGChannels(k));
            end
            legend(leg);
        else
            switch (size(perChannelCorr,2))     % legacy format - no longer used
    ...
    elseif (~showMag)
        ...
        if (isfield(acs, 'AWGChannels') &&  ~isempty(acs.AWGChannels))
            leg = {};
            for k = 1:length(acs.AWGChannels)
                leg{k} = sprintf('Phase/Ch%d', acs.AWGChannels(k));
            end
            legend(leg);
        else
            switch (size(perChannelCorr,2))     % legacy format - no longer used
        ...
        if (isfield(acs, 'AWGChannels') &&  ~isempty(acs.AWGChannels))
            ...
            for k = 1:length(acs.AWGChannels)
                leg{k+length(acs.AWGChannels)} = sprintf('Phase/Ch%d', acs.AWGChannels(k));
            end
            legend(leg);
        else
            switch (size(perChannelCorr,2))     % legacy format - no longer used
...
try
    acs.sparamCutoff = iqparse(get(handles.editSParamCutoff, 'String'), 'scalar');
catch
    acs.sparamCutoff = 0;
end
save(ampCorrFile, '-struct', 'acs');
...
h = msgbox('Loading frequency/phase response calibration data from instrument...', 'Please wait', 'replace');
if ((~isempty(strfind(arbConfig.model, 'M8195A')) || ~isempty(strfind(arbConfig.model, 'M8196A')) || ...
    ...
    try
        ...
        for i = chList
            if (i < 5)
                ...
                va = sprintf('visaAddr%d', floor((i-1)/4+1));
            end
            f = iqopen(arbConfig.(va));
            a = xquery(f, sprintf(':SOUR:CHAR%d?', mod(i-1, 4)+1));
            iqclose(f);
            if (isempty(a) || strncmp(a, '""', 2))
                error(sprintf('no frequency response for channel %d available', i));
            end
            ...
            if (isempty(freq))
                ...
                if (~isequal(v(:,1), freq))
                    error('frequency responses must have the same frequency points for all channels');
                end
            end
            cplxCorr(:,i) = v(:,2) .* exp(1j * v(:,3));
        end
        updatePerChannelCorr(handles, freq, cplxCorr, chList');
    catch ex
        errordlg({ex.message, [ex.stack(1).name ', line ' num2str(ex.stack(1).line)]});
    end
elseif (~isempty(strfind(arbConfig.model, 'M8199A')) || ~isempty(strfind(arbConfig.model, 'M8199B')))
    f = iqopen(arbConfig);
    if (~isempty(f))
        try
            chList = find(arbConfig.channelMask);
            chList2 = chList;
            prompt = 'Overwrite';  % overwrite for the first channel
            for i = chList
                a = xquery(f, sprintf(':SOUR:CHAR:FACT? "%s"', buildID(arbConfig, i)));
                if (isempty(a) || strncmp(a, '""', 2))
                    chList2(chList2 == i) = [];
                    continue;
                end
                v = sscanf(strrep(strrep(a, '"', ''), ',', ' '), '%g');
                v = reshape(v, 3, length(v)/3)';
                if (exist('cplxCorr', 'var') && (size(v,1) ~= size(cplxCorr,1) || max(abs(v(:,1) - freq)) > 1e-3))
                    chList2(chList2 == i) = [];
                    continue;
                end
                updatePerChannelCorr(handles, v(:,1), v(:,2) .* exp(1j * v(:,3)), i, prompt);
                prompt = 'Merge';  % use merge for all of the subsequent channels
            end
        catch ex
            errordlg({ex.message, [ex.stack(1).name ', line ' num2str(ex.stack(1).line)]});
        end
    ...
    errordlg({'Only the M8195A/96A/94A/99A/99B support built-in correction data'});
end
...
if (isfield(acs, 'perChannelCorr') && size(acs.perChannelCorr, 2) > 2)
    ...
    res = inputdlg(sprintf('Select the list of channels to clear\n(separate channel numbers by space or comma)'), ...
                     'Select the channels to clear', 1, chanStr);
    if (isempty(res))
        return;
    end
...
% remove amplitude dependent phase to avoid unexpected behavior
acs.nonLinCorr = [];
acs = rmfield(acs, 'nonLinCorr');
% and save
save(ampCorrFile, '-struct', 'acs');
updateAxes(handles);           
    ...
    switch filterindex
        ...
        case 1 % S-parameter file
            ...
            try
                ...
                freq = sp.Freq;
                defaultVal = {'2 1'};
                if (size(sp.S_Parameters, 1) == 4)
                    defaultVal = {'2 1  4 3'};
                elseif (size(sp.S_Parameters, 1) == 6)
                    defaultVal = {'2 1  4 3  6 5'};
                elseif (size(sp.S_Parameters, 1) == 8)
                    defaultVal = {'2 1  4 3  6 5  8 7'};
                end
                result = [];
...
Ns2p = sprintf('%d Touchstone 2-port files (*.s2p)', numChan);
...
if (filename ~= 0)
    switch filterindex
        ...
        case 6 % .cal (VSA User Correction)
            try
                ch = 1;
                if (size(perChannelCorr, 2) > 2)
                    list = {'Primary / I', 'Secondary / Q', '3rd', '4th'};
        ...
        case 2 % nChannel .s2p files
            try
                ...
                for i = 1:numChan
                    ...
                    sparam(2,1,:) = tmp;
                    sp = rfdata.data('Freq', freq, 'S_Parameters', sparam);
                    sp.write(fullfile(pathname, sprintf('%s_%d%s',filenameStr,i,extStr)));
                end
            catch ex
                errordlg({ex.message, [ex.stack(1).name ', line ' num2str(ex.stack(1).line)]});
            end
...
value = [];
try
    value = iqparse(get(handles.editAbsMagnitude, 'String'), 'vector');
catch ex
    msgbox(ex.message);
end
...
try
    ...
    weight = iqparse(get(handles.editWeight, 'String'), 'vector');
