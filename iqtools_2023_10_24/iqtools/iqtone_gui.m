...
% Last Modified by GUIDE v2.5 22-Jan-2020 17:10:49
...
switch arbConfig.model
    ...
    case { 'M8195A_Rev0' 'M8195A_Rev1' 'M8195A_1ch' 'M8195A_2ch_256k' 'M8195A_4ch_256k' }
        ...
        notchSpan = 10e6;
        numTones = 100;
    case { 'M8195A_2ch', 'M8195A_2ch_mrk', 'M8195A_2ch_dupl' }
        startFreq = 10e7;
        stopFreq = 10e9;
        notchFreq = 5e9;
        notchSpan = 100e6;
...
% place tone spacing and compression on top of each other - only one is
% needed at a time
pos = get(handles.textToneSpacing, 'Position');
set(handles.textCompression, 'Position', pos);
pos = get(handles.editToneSpacing, 'Position');
set(handles.editCompression, 'Position', pos);
...
if (isscalar(value) && value >= 0 && value <= 100000)
    points = iqparse(get(handles.editPoints, 'String'), 'scalar');
    numTones = iqparse(get(handles.editTones, 'String'), 'scalar');
    startFreq = iqparse(get(handles.editStartFreq, 'String'), 'vector');
    stopFreq = iqparse(get(handles.editStopFreq, 'String'), 'vector');
    startFreq = startFreq(1); % in case the user entered a list
    stopFreq = stopFreq(1);   % in case the user entered a list
    if (numTones == 0)
        set(handles.editCompression, 'Visible', 'on');
        set(handles.textCompression, 'Visible', 'on');
    ...
    elseif (numTones == 1)
        set(handles.editCompression, 'Visible', 'off');
        set(handles.textCompression, 'Visible', 'off');
        set(handles.editToneSpacing, 'String', 'n/a');
...
if (numTones == 0)
    figure(7);
    histogram(gca(), real(iqdata), 1000);
end
...
correction = get(handles.checkboxCorrection, 'Value');
freqOffset = iqparse(get(handles.editCarrier, 'String'), 'scalar');
...
mag = iqparse(get(handles.editMagnitude, 'String'), 'vector');
...
try
    ...
    % perChannelCorrection does not work properly with negative
    % frequencies
    if (min(tone) < 0)
        pcc = 0;
    else
    ...
    switch (get(handles.popupmenuCalType, 'Value'))
        case 1  % spectrum analyzer (zero span)
            res = iqcal('tone', tone, 'offset', freqOffset, 'recalibrate', recal, 'method', 'zero span', 'samplerate', fs, 'chMap', channelMapping, ...
                'normalize', normalize, 'correction', correction, 'magnitude', mag, 'handles', handles, 'msgbox', hMsgBox, 'perChannelCorr', pcc);
...
startFreq = iqparse(get(handles.editStartFreq, 'String'), 'vector');
stopFreq = iqparse(get(handles.editStopFreq, 'String'), 'vector');
notchFreq = iqparse(get(handles.editNotchFreq, 'String'), 'vector');
notchSpan = iqparse(get(handles.editNotchSpan, 'String'), 'vector');
notchDepth = iqparse(get(handles.editNotchDepth, 'String'), 'vector');
notchEnable = get(handles.checkboxNotchEnable, 'Value');
if (numTones == 1)
...
mag0 = iqparse(get(handles.editMagnitude, 'String'), 'vector');
mag0 = fixlength(mag0, numTones)';
mag = mag + mag0;
% if we generate MATLAB code, return a string instead of a vector of tones
numTones = iqparse(get(handles.editTones, 'String'), 'scalar');
...
if (phaseIdx == 5)
    phase = iqparse(get(handles.editPhase, 'String'), 'vector');
    phase = phase ./ 180 .* pi;
end
correction = get(handles.checkboxCorrection, 'Value');
channelMapping = get(handles.pushbuttonChannelMapping, 'UserData');
...
if (numTones == 0)
    ...
    notchDepth = iqparse(get(handles.editNotchDepth, 'String'), 'vector');
    notchEnable = get(handles.checkboxNotchEnable, 'Value');
    compression = iqparse(get(handles.editCompression, 'String'), 'vector');
    if (notchEnable == 0)
        notchFreq = [];
        notchSpan = [];
        notchDepth = [];
    ...
    if (exist('doCode', 'var'))
        ...
        iqdata = sprintf(['fs = %s;\n[iqdata, chMap] = iqnoise(''sampleRate'', fs, ...\n' ...
            ...
            '    ''correction'', %d, ''compression'', %g, ''channelMapping'', %s);\n' ...
            'iqdownload(iqdata, fs, ''channelMapping'', chMap, ...\n' ...
            '    ''segmentNumber'', %d);\n'], ...
            iqengprintf(sampleRate), points, startFreqStr, stopFreqStr, notchFreqStr, ...
            notchSpanStr, notchDepthStr, correction, compression, chanMapStr, segmentNum);
    else
        [iqdata, channelMapping] = iqnoise('sampleRate', sampleRate, 'numSamples', points, ...
            ...
            'compression', compression, 'channelMapping', channelMapping);
...
value = [];
try
    value = iqparse(get(handles.editPoints, 'String'), 'scalar');
catch ex
    msgbox(ex.message);
    result = 0;
end
...
if (length(find(sum(channelMapping, 1))) >= 2)
    ...
    if (length(tone) == 1)
        span = iqparse(get(handles.editSampleRate, 'String'), 'scalar') / 2;
        fc = span/2;
    end
else
