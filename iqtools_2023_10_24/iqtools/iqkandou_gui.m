...
symbolShift = 0; %iqparse(get(handles.editSymbolShift, 'String'), 'scalar');
...
isi = 0; %iqparse(get(handles.editISI, 'String'), 'scalar');
amplitude = 1; %iqparse(get(handles.editAmplitude, 'String'), 'scalar');
...
marker = [];
div = round(iqparse(get(handles.editClockDivider, 'String')), 'scalar');
isCheckedOnce = 0;
numBits = iqparse(get(handles.editNumBits, 'String'), 'scalar');
if (div > 1)
...
dr = iqparse(get(handles.editDataRate, 'String'), 'scalar');
numBits = iqparse(get(handles.editNumBits, 'String'), 'scalar');
...
if (refclk)
    ...
    clkdiv = iqparse(get(handles.editClockDivider, 'String'), 'scalar');
    clkFreq = dr / clkdiv;
end
if (arbConfig.isDCAConnected)
    f = iqopen(arbConfig.visaAddrDCA);
    if (~isempty(f))
        xfprintf(f, ':TRIG:PLOC OFF');
        xquery(f, '*OPC?');
        xfprintf(f, ':TIM:PTIM:RFR %g', clkFreq);
        xfprintf(f, ':TIM:PTIM:RTR');
        xfprintf(f, ':TRIG:SOUR FPAN');
        xfprintf(f, ':TRIG:MODE CLOCK');
        xfprintf(f, ':TRIG:SRAT:AUTO OFF');
        xfprintf(f, ':TRIG:SRAT %g', dr);
        xfprintf(f, ':TRIG:PLEN:AUTO OFF');
        xfprintf(f, ':TRIG:PLEN %d', numBits);
        xfprintf(f, ':TRIG:DCDR:AUTO OFF');
        xfprintf(f, ':TRIG:DCDR SUB%d', clkdiv);
        xfprintf(f, ':TRIG:PLOC ON');
        xquery(f, '*OPC?');
    end
else
