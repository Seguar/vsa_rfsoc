...
%                 arbSync.visaAddr = arbConfig.visaAddrM8192A;
    ...
    if (~isempty(strfind(opts, '002')) || ~isempty(strfind(opts, 'R12')))
        if (strcmp(arbConfig.model, 'M8195A_2ch_mrk') || strcmp(arbConfig.model, 'M8195A_2ch_dupl'))
    ...
    if (segmentOffset == 0)
        ...
        switch (arbConfig.model)
            ...
            case 'M8195A_2ch_dupl'
                ...
                chMap(4,:) = zeros(1,size(chMap, 2));
                fsDivider = 2;
                xfprintf(f, sprintf(':INST:DACM DCD;:TRAC1:MMOD EXT;:TRAC2:MMOD EXT;:INST:MEM:EXT:RDIV DIV%d', fsDivider));
                xfprintf(f, ':OUTP3 ON;:OUTP4 ON');
            case 'M8195A_4ch'
                fsDivider = 4;
...
function setTrigMode(arbConfig, f)
    contMode = -1;
    gateMode = -1;
...
if (segm_len > 0)
    ...
    if (isfield(arbConfig, 'skew'))
        ...
        if (tmp(chan) ~= 0)
            if (segmentOffset ~= 0)
                warndlg('Delay cannot be applied when block-wise download is used', 'Delay Warning', 'replace');
            else
    ...
    if (strcmp(arbConfig.model, 'M8195A_1ch') || ...
        strcmp(arbConfig.model, 'M8195A_1ch_mrk') || ...
...
switch (seqcmd.cmd)
    case 'list'
        s = sscanf(xquery(f, sprintf(':TRACe%d:CATalog?', find(channelMapping(:,1) + channelMapping(:,2), 1))), '%d,');
    ...
    case 'triggerMode'
        ...
        xfprintf(f, sprintf(':INIT:GATE 0'));
    case 'stop'
        for i = find(channelMapping(:,1) + channelMapping(:,2))'
            xfprintf(f, sprintf(':ABORt%d', i));
        end
...
if (~isempty(find(channelMapping(:,1) + channelMapping(:,2), 1)))
    s = sscanf(xquery(f, sprintf(':trac%d:cat?', find(channelMapping(:,1) + channelMapping(:,2), 1))), '%d,');
    s = reshape(s,2,length(s)/2);
    notDef = [];
...
xfprintf(f, ':ABORt');
stab = xquery(f, sprintf(':STAB:DATA? %d,%d', start, 6*len));
