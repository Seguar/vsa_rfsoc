...
% If the fc is 0, but we are generating all positive frequencies, set fc to
% some non-zero value to force vsafunc('input', ...) to switch the scope to
% baseband mode instead of I+jQ
if (fc == 0)
  if (min(tone) > 0)
      fc = 1e-6;
  end
  ...
  %carrierOffset = (min(tone)+max(tone))/2;
end
