...
% close the session after each command
...
while (i <= nargin)
    if (ischar(varargin{i}))
        switch lower(varargin{i})
            ...
            case 'udp';             useUDP = str2double(varargin{i+1});
            case 'restart';         restart = str2double(varargin{i+1});
            case 'closeimmediate';  closeImmediate = str2double(varargin{i+1});
            case 'showcommands';    showCommands = str2double(varargin{i+1});
...
try  
    while (1)
        ...
        if useUDP
            ...
            serverInstance = IQToolsServer(ipAddressThisPC, UDPReceivePort, closeImmediate);
            ...
            h.update(0, sprintf('Server waiting for connection (TCP port %d)...\nPress CTRL+C in the Workspace Window to close.', TCPIPPort));
            serverInstance = IQToolsServer(ipAddressThisPC, TCPIPPort, closeImmediate);
        ...
        cnt = 0;
        while (isvalid(serverInstance.Server.ServerInstance) && strcmp(serverInstance.Server.ServerInstance.Status, 'open'))
            cnt = cnt + 1;
