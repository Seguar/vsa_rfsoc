...
switch (arbConfig.model)
    case {'M8195A_2ch' 'M8195A_2ch_mrk' 'M8195A_2ch_256k' 'M8195A_2ch_dupl'}
...
% f = iqopen();
% if (~isempty(f))
%     res = str2double(xquery(f, ':FREQ:RAST?'));
%     handles.fs = res;
%     % Update handles structure
%     guidata(hObject, handles);
%     iqclose(f);
...
for ch = 1:4
    if (channels(ch))
        xfprintf(f, [sprintf(':OUTP%d:FILT:%s %.9g', ch, cmds{os}, filt(1)) sprintf(',%.9g', filt(2:end))]);
    end
end
...
res = int16(str2double(xquery(f, ':STAT:QUES:VOLT:COND?')));
ch = '';
if (bitand(res, 16) ~= 0)
    ch = [ch '1, '];
end
...
%         fstr = design(fdes);
%         fNum = fstr.Numerator;
        fNum = rcosdesign(filterParam, 16, os, 'normal');
        m = max(abs(fNum));
        fNum = fNum / m;
    case 'Allpass'
        fNum = zeros(1,os*16);
...
value = [];
try
    value = iqparse(get(handles.editParam, 'String'), 'scalar');
catch
end
if (~isempty(value))
...
value = [];
try
    value = iqparse(get(handles.editDelay, 'String'), 'scalar');
catch
end
...
value = [];
try
    value = iqparse(get(handles.editDelayTime, 'String'), 'scalar');
catch
end
try
    fs = handles.fs;
...
value = [];
try
    value = iqparse(get(handles.editScale, 'String'), 'scalar');
catch
end
