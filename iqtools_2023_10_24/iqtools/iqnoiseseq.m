...
try
    ...
    for ch = find(channelMapping(:,1) + channelMapping(:,2))';
        xfprintf(f, sprintf(':ABORt%d', ch));
        xbinblockwrite(f, seq, 'uint32', sprintf(':STAB%d:DATA 0,', ch));
        xfprintf(f, sprintf(':STABle%d:SEQuence:SELect %d', ch, 0));
        xfprintf(f, sprintf(':STABle%d:DYNamic:STATe 0', ch));
        xfprintf(f, sprintf(':FUNCtion%d:MODE STSC', ch));
    end
    xfprintf(f, sprintf(':INIT:IMM'));
    res = strtrim(xquery(f, ':SYST:ERR?'));  % must issue a query before closing to flush the buffer
    iqclose(f);
catch(ex)
end
delete(hMsgBox);
