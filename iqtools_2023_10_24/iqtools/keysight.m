...
if (wiggling)
    ...
    if (doDownload)
        ...
        iqseq('mode', 'STSC');
        f = iqopen();
        xfprintf(f, sprintf(':mark1:samp:volt:ampl %g; offs %g', 500e-3, 0));
        xfprintf(f, sprintf(':mark1:sync:volt:ampl %g; offs %g', 500e-3, 0));
    end
end
if (doDownload)
    try
...
if (doDownload)
    ...
    if ((isfield(arbConfig, 'isScopeConnected') && arbConfig.isScopeConnected == 0) || ~isfield(arbConfig, 'visaAddrScope'))
        ...
        if (~isempty(f))
            xfprintf(f, '*rst');
            xfprintf(f, sprintf(':timebase:scal %g', duration / 12));
            xfprintf(f, ':timebase:delay 0');
            xfprintf(f, sprintf(':chan%d:disp on;  scale %g; offs 0', 1, 100e-3));
            xfprintf(f, sprintf(':chan%d:disp off; scale %g; offs 0', 2, 100e-3));
            xfprintf(f, sprintf(':chan%d:disp off; scale %g; offs 0', 3, 100e-3));
            xfprintf(f, sprintf(':chan%d:disp off; scale %g; offs 0', 4, 100e-3));
            xfprintf(f, sprintf(':trig:edge:source chan2'));
            xfprintf(f, sprintf(':trig:edge:slope pos'));
            xfprintf(f, sprintf(':trig:lev chan2,0'));
            xfprintf(f, sprintf(':acquire:average:count 2'));
            xfprintf(f, sprintf(':acquire:average %s', '1'));
            xquery(f, '*opc?');
            iqclose(f);
        end
    end
