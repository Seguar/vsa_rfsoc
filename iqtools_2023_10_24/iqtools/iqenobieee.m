...
%     end
    ...
    try
        ...
        xfprintf(awg, 'ABOR');
        xfprintf(awg, sprintf('FREQ:RAST %e ',fsAWG));
        % ---

        % set up the timebase channel
        xfprintf(awg, sprintf(':TRAC%d:DEL 1',awg_timebase_channel));
        xfprintf(awg, sprintf(':TRAC%d:DEF 1,%d,0',awg_timebase_channel,length(trig_samples)));
        xbinblockwrite(awg,trig_samples,'int8',sprintf(':TRAC%d:DATA 1,0,',awg_timebase_channel));
        xfprintf(awg, sprintf('VOLT%d 0.5',awg_timebase_channel)); % use 1V amplitude
        xfprintf(awg, sprintf('OUTP%d ON',awg_timebase_channel));
        % ---

% we use the same channel (P/N) for trigger and timebase so this is redundant
%         % setup the trigger channel
%         xfprintf(awg, sprintf(':TRAC%d:DEL 1',awg_trigger_channel));
%         xfprintf(awg, sprintf(':TRAC%d:DEF 1,%d,0',awg_trigger_channel,length(trig_samples)));
%         xbinblockwrite(awg,trig_samples,'int8',sprintf(':TRAC%d:DATA 1,0,',awg_trigger_channel));
%         xfprintf(awg, sprintf('VOLT%d 0.5',awg_trigger_channel)); % use 1V amplitude
%         xfprintf(awg, sprintf('OUTP%d ON',awg_trigger_channel));
%         % ---
        ...
        xfprintf(awg, sprintf(':TRAC%d:DEL 1',awg_meas_channel));
        xfprintf(awg, sprintf(':TRAC%d:DEF 1,%d,0',awg_meas_channel,length(samples)));
        xbinblockwrite(awg,samples,'int8',sprintf(':TRAC%d:DATA 1,0,',awg_meas_channel));
        xfprintf(awg, sprintf('VOLT%d %f',awg_meas_channel,amplitude));
        xfprintf(awg, sprintf('OUTP%d ON',awg_meas_channel));
        opc = xquery(awg,':INIT:IMM; *OPC?'); %#ok<*NASGU>
        % ---
        ...
        xfprintf(dca, '*CLS');
        if scopeRST == 1
            opc = xquery(dca, '*RST; *OPC?');
        end    
        xfprintf(dca, ':SYST:MODE OSC');
        % ---
        
        % get the module model
        pattern = '\w*(\d)\w?';
        slotnumber = regexp(dca_meas_channel,pattern,'tokens');
        model = xquery(dca, sprintf(':SYST:MOD? SLOT%s',slotnumber{1}{1}));
        % ---
               
        % use the complete acquisition memory
        xfprintf(dca, ':ACQ:RLEN:MOD MAN');
        xfprintf(dca, sprintf(':ACQ:RLEN %d', dca_mem_depth));
        ...
        if isempty(strfind(opt, 'PTB'))
            ...
            % in slots 3+4
            xfprintf(dca, ':PTIM3:RFR:AUT OFF');
            xfprintf(dca, ':PTIM3:RMET OLIN');
            xfprintf(dca, sprintf(':PTIM3:RFR %e',ftrig));
            opc = xquery(dca, ':PTIM3:STAT ON; *OPC?');
        else
            % use the internal PTB of the mainframe if available
            xfprintf(dca, ':TIM:PTIM:RFR:AUT OFF');
            xfprintf(dca, ':TIM:PTIM:RMET OLIN');
            xfprintf(dca, sprintf(':TIM:PTIM:RFR %e',ftrig));
            opc = xquery(dca, ':TIM:PTIM:STAT ON; *OPC?');
        end

        % setup the trigger
        xfprintf(dca, ':TRIG:SOUR FPAN');
        xfprintf(dca, ':TRIG:MOD CLOC');
        xfprintf(dca, ':TRIG:BRAT:AUT OFF');
        xfprintf(dca, sprintf(':TRIG:BRAT %e', fsAWG));
        xfprintf(dca, ':TRIG:PLEN:AUT OFF');
        xfprintf(dca, sprintf(':TRIG:PLEN %d', length(samples)));
        xfprintf(dca, ':TRIG:DCDR:AUT ON');
        opc = xquery(dca, ':TRIG:PLOC ON; *OPC?');
        % ---
        ...
        if isempty(strfind(opt, 'PTB'))
           opc = xquery(dca, ':PTIM3:RFR:AUT ON; *OPC?'); 
        else
           opc = xquery(dca, ':TIM:PTIM:RFR:AUT ON; *OPC?');
        end
        % ---
        ...
        if dca_diff_mode == 1 % differential
            xfprintf(dca, sprintf(':%s:DMOD ON',chan));           
            chan = xquery(dca, sprintf('%s:FCH?',chan));            
        end
        xfprintf(dca, sprintf(':%s:DISP ON',dca_channel_id)); 
        if not(isempty(strfind(model,'N1045A')))
            xfprintf(dca, sprintf(':%s:BAND BAND4',chan)); % 60 GHz
        elseif not(isempty(strfind(model,'86118A')))
            xfprintf(dca, sprintf(':%s:BAND BAND1',chan)); % 50 GHz
        else
            xfprintf(dca, sprintf(':%s:BAND HIGH',chan));
        end
        % ---
        
        % perform automatic deskew if making a differential measurement
        if dca_diff_mode == 1 % differential
            opc = xquery(dca, sprintf(':%s:DESkew;*OPC?',dca_channel_id));
        end
        % ---
                
        % set up averaging if desired
        xfprintf(dca, ':ACQ:STOP');
        if Navg > 1
            xfprintf(dca, ':ACQ:SMO AVER');
            xfprintf(dca, sprintf(':ACQ:ECO %d', Navg));
        else
            xfprintf(dca, ':ACQ:SMO NONE');
        end
        % ---
        ...
        % output data stream.
        xfprintf(dca, ':LTES:ACQ:CTYP PATT');
        xfprintf(dca, sprintf(':LTES:ACQ:CTYP:PATT %d', Nshots));
        xfprintf(dca, ':LTES:ACQ:STAT ON');
        % ---
...
%                 waitbar((2*nn)/(2*length(tones)), hMsgBox, 'Reading data from DCA...');
                hMsgBox.update((2*nn)/(2*length(tones)), 'Reading data from DCA...');
            end
            % perform autoscale
            opc = xquery(dca, 'SYST:AUT; *OPC?');

            % set up the measurement
            xfprintf(dca, ':TIM:UNIT BIT');
            xfprintf(dca, sprintf(':TIM:BRAT %e', fsAWG));
            xfprintf(dca, ':TIM:MPOS 24.1E-9');
            xfprintf(dca, ':TIM:POS 24.1E-9');

            opc = xquery(dca, ':ACQ:EPAT ON; *OPC?');
            xfprintf(dca, ':ACQ:RSP SPB');
            xfprintf(dca, ':ACQ:SPB:MOD MAN');
            opc = xquery(dca, sprintf(':ACQ:SPB %d; *OPC?',spb));  
            % ---
                      
            % measure the test signal
            dca.Timeout = 900; % this will take some time, so increase the timeout
            xfprintf(dca, ':ACQ:CDIS');
            opc = xquery(dca, ':ACQ:RUN; *OPC?');
            % ---

            % get the waveform
            xfprintf(dca, ':SYST:BORD LEND');
            xfprintf(dca, sprintf(':WAV:SOUR %s',dca_channel_id));
%             xfprintf(dca, ':WAV:SOUR FUNC1');
            
            ydata_raw = xbinblockread(dca, 'double', ':WAV:XYF:DOUB:YDAT?'); % in Volts

            xdata_raw = xbinblockread(dca, 'double', ':WAV:XYF:DOUB:XDAT?'); % in Volts
            dca.Timeout = 240;
            % ---
