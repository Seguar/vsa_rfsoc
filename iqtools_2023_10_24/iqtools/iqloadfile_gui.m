...
try
    ...
    fclose(f);
        ...
        % for .csv, try to read from the file itself
        if (get(handles.popupmenuType, 'Value') == 1) % CSV
            file2 = filename;
        else
            file2 = [filename '.txt'];
        end
...
if (~isempty(iqdata) && ~isempty(fs) && (fs ~= 0))
...
if (~isempty(iqdata))
    segmentNum = iqparse(get(handles.editSegment, 'String'), 'scalar', 'integer');
...
%    mtype = mtypeList{get(handles.popupmenuMarkerColumn, 'Value')};
    ...
    if (get(handles.checkboxResample, 'Value'))
        ...
        resampleFactor = iqparse(get(handles.editResampleFactor, 'String'), 'scalar');
    else
        resampleMethod = [];
        resampleMethodStr = '[]';
        resampleFactor = 1;
    ...
    if (get(handles.checkboxWindow, 'Value'))
        ...
        windowFactor = iqparse(get(handles.editWindowFactor, 'String'), 'scalar');
    else
        windowMethod = [];
        windowMethodStr = '[]';
        windowFactor = 0;
    ...
    if (get(handles.checkboxFreqShift, 'Value'))
        frequencyShift = iqparse(get(handles.editFc, 'String'), 'scalar', 'nothrow');
    else
        frequencyShift = [];
    end
    ...
    if (~normalize)
        ...
        minScale = iqparse(get(handles.editScaleToMin, 'String'), 'scalar');
        maxScale = iqparse(get(handles.editScaleToMax, 'String'), 'scalar');
...
factor = iqparse(get(handles.editResampleFactor,'String'), 'scalar');
sampleRate = iqparse(get(handles.editSampleRate, 'String'), 'scalar');
...
value = -1;
try
    value = iqparse(get(hObject, 'String'), 'scalar', 'integer');
catch ex
    errordlg({ex.message, [ex.stack(1).name ', line ' num2str(ex.stack(1).line)]});
end
arbConfig = loadArbConfig();
...
maxVal = 1.1;
try
    minVal = iqparse(get(handles.editScaleToMin, 'String'), 'scalar');
    maxVal = iqparse(get(handles.editScaleToMax, 'String'), 'scalar');
catch ex
