...
% THIS SOFTWARE MAY ONLY BE USED IN CONJUNCTION WITH KEYSIGHT INSTRUMENTS. 
    ...
    if (~isempty(strfind(opts, 'LPN')))
        lpn = str2double(xquery(f, ':FREQ:RAST:LPN:ACT?'));
    else
        lpn = 0;
    end
...
% seqcmd.sequence contains the parameter(s)
    ...
    switch (seqcmd.cmd)
        ...
        case 'actionDefine'
            ...
            for i = find(channelMapping(:,1) + channelMapping(:,2))'
                xfprintf(f, sprintf(':ABORt%d', i));
                result = str2double(xquery(f, sprintf(':ACTion%d:DEFine:NEW?', i)));
            end
...
function result = readSequence(f, seqcmd, channelMapping)
    ...
    stab = xquery(f, sprintf(':STAB%d:DATA? %d,%d', ch, start, 6*len));
