...
if (isempty(f))
    ...
    try
        a = xquery(f, ':carr:freq?');
        b = eval(['[' strtrim(a) ']']);
        c = eval(xquery(f, ':carr:scale?'));
...
% handles    structure with handles and user data (see GUIDATA)
if (iqoptcheck([], 'DUC', 'DUC'))
    cycleTime = iqparse(get(handles.editCycleTime, 'String'), 'scalar');
...
try
f = iqopen(arbConfig);
s = sscanf(xquery(f, sprintf(':TRACe%d:CATalog?', 1)), '%d,');
...
if ((~isfield(arbConfig, 'isScopeConnected') || (isfield(arbConfig, 'isScopeConnected') && arbConfig.isScopeConnected ~= 0)) && isfield(arbConfig, 'visaAddrScope'))
    ...
    f = iqopen(scopeCfg);
    if (~isempty(f))
        xfprintf(f, ':meas:clear');
        offs = 0;
        scale = 100e-3;
        for i = [1 2]
            xfprintf(f, sprintf(':chan%d:disp on', i));
            xfprintf(f, sprintf(':chan%d:offs %g', i, offs));
            xfprintf(f, sprintf(':chan%d:scale %g', i, scale));
        end
        xfprintf(f, sprintf(':timebase:scal %g', 50e-9));
        xfprintf(f, sprintf(':trig:mode edge'));
        xfprintf(f, sprintf(':trig:edge:slope positive'));
        xfprintf(f, sprintf(':trig:hold 30e-6'));
        trigLev = 100e-3;
        xfprintf(f, sprintf(':trig:edge:source chan2'));
        xfprintf(f, sprintf(':trig:lev chan2,%g', trigLev));
        xfprintf(f, sprintf(':timebase:del %g', 175e-9));
        xfprintf(f, ':run');
        res = xquery(f, ':syst:err?');
        disp(res);
        iqclose(f);
    end
end
