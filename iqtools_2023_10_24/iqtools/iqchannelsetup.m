        ...
        if (isempty(ud))
            ...
            elseif (arbConfig.numChannels >= 2)
                if (strcmp(arbConfig.model, 'M8195A_2ch') || strcmp(arbConfig.model, 'M8195A_2ch_256k'))
                    ud = [1 0; 0 0; 0 0; 0 1];
                elseif (strcmp(arbConfig.model, 'M8195A_2ch_mrk') || strcmp(arbConfig.model, 'M8195A_2ch_dupl') )
                    ud = [1 0; 0 1; 0 0; 0 0];
...
%             ud = [1 0; 0 0; 0 0; 0 1];
%         end
        duc = (~isempty(strfind(arbConfig.model, 'DUC')) || ~isempty(strfind(arbConfig.model, 'modulated')) || isfield(arbConfig, 'IQonly'));
        if (duc)
            ud(:,1) = (ud(:,1) + ud(:,2)) ~= 0;
            ud(:,2) = ud(:,1);
        else
