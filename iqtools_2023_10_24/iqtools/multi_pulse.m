...
while (i <= nargin)
    if (ischar(varargin{i}))
        switch lower(varargin{i})
            ...
            case 'debugmode';      debugmode = varargin{i+1};
            case 'processfile';    processFile = varargin{i+1};
...
totalSamples = arbConfig.minimumSegmentSize;
...
for i=1:pulseTableLen
    ...
    if (~isfield(pulseTable(i), 'scanType') || isempty(pulseTable(i).scanType))
        pulseTable(i).scanType = 'None';
        pulseTable(i).scanFct = [];
        pulseTable(i).scanPeriod = [];
        pulseTable(i).scanAz = [];
        pulseTable(i).scanSq = [];
    end
...
for i=1:pulseTableLen
    ...
    pulseTable(i).handle = iqsteppulse(...
        ...
        'PW', pulseTable(i).PW, 'risetime', pulseTable(i).riseTime, ...
        'falltime', pulseTable(i).fallTime, 'offset', pulseTable(i).offset, ...
        'modulationType', pulseTable(i).modulationType, ...
        'span', pulseTable(i).span, 'amplitude', pulseTable(i).amplitude, 'normalize', 0, ...
...
for i=1:toneTableLen
    ...
    toneTable(i).handle = iqsteptone(...
        ...
        'magnitude', toneTable(i).amplitude, 'normalize', 0, 'correction', correction);
end
...
for i=1:pulseTableLen
    ...
    pulseTable(i).riseTime = fixlength(pulseTable(i).riseTime, pulseTable(i).numPulse);
    pulseTable(i).fallTime = fixlength(pulseTable(i).fallTime, pulseTable(i).numPulse);
    pulseTable(i).riseTs = round(pulseTable(i).riseTime * sampleRate);
    pulseTable(i).fallTs = round(pulseTable(i).fallTime * sampleRate);
    ...
    pulseTable(i).amplitude = fixlength(pulseTable(i).amplitude, pulseTable(i).numPulse);
    pulseTable(i).sumprs = round(sum(pulseTable(i).PRI) * sampleRate);
    pulseTable(i).scanOffset = rand(1,1);
    switch (pulseTable(i).scanType)
        case 'None'
...
%                 cumsum(pulseTable(i).prs) + currps + ...
%                 max(pulseTable(i).pws + pulseTable(i).riseTs + pulseTable(i).fallTs, 6168); % 24 * 257
...
%     else
        ...
        pwv = repmat(pulseTable(i).pws + pulseTable(i).riseTs + pulseTable(i).fallTs, 1, numGroups);
...
%     end
    ...
    ampl = unique(pulseTable(i).amplitude);
    if (~isscalar(ampl))
...
for i = 1:numEntries
    ...
    for k = 1:pulseTable(i).numPulse
        ...
        pri = max(24 * 257 / sampleRate, ...
            pulseTable(i).PW(k) + pulseTable(i).riseTs(k) + pulseTable(i).fallTs(k) + 1e-10);
        ...
        iqdata = iqpulse('arbConfig', arbConfig, ...
            ...
            'risetime', pulseTable(i).riseTime(k), ...
            'falltime', pulseTable(i).fallTime(k), ...
...
%    atab(i) = iqseq('actionDefine', { 'AMPL', ampl });
    atab(i) = str2double(xquery(f, ':ACTion1:DEFine:NEW?'));
    xfprintf(f, sprintf(':ACTion1:APPend %d,%s,%.15g', atab(i), 'AMPL', ampl));
    atab(i) = str2double(xquery(f, ':ACTion2:DEFine:NEW?'));
...
try
    ...
    if (~isempty(strfind(arbConfig.model, 'DUC')))
        f = iqopen();
        if (~isempty(f))
            xfprintf(f, sprintf(':carr1:freq %.0f,%g', floor(fc), fc - floor(fc)));
            xfprintf(f, sprintf(':carr2:freq %.0f,%g', floor(fc), fc - floor(fc)));
            iqclose(f);
        end
    end
    if (saConfig.connected)
        ...
        if (~isempty(f))
            ...
            resbw = 300e3;
            xfprintf(f, '*cls');
            res = xfprintf(f, ':inst rtsa', 1);
            if (res == 0) % RTSA is supported
                xfprintf(f, sprintf(':FREQuency:CENTer %g', fc));
                xfprintf(f, sprintf(':FREQuency:SPAN %g', 160e6));
            else
                xfprintf(f, sprintf(':FREQuency:CENTer %g', fc_for_sa));
                xfprintf(f, sprintf(':BWID %g', resbw));
                xfprintf(f, sprintf(':BWID:VID:AUTO ON'));
                if (sweeptime ~= 0)
                    xfprintf(f, sprintf(':SWEep:TIME %g', sweeptime));
                    xfprintf(f, sprintf(':FREQuency:SPAN %g', 0));
                else
                    xfprintf(f, sprintf(':SWEep:TIME:AUTO ON'));
                    xfprintf(f, sprintf(':FREQuency:SPAN %g', span));
                end
                xfprintf(f, sprintf(':INIT:RESTart'));
            end
            iqclose(f);
        else
