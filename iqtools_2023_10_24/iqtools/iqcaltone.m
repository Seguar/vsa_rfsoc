        ...
        if (runFlag)
            ...
            if (~isempty(fAWG))
                for ch = find(sum(channelMapping, 2))'
                    xfprintf(fAWG, sprintf(':VOLT%d %.15g', ch, amp));
                end
                iqclose(fAWG);
            end
        end
...
for nt = 1:length(tones)
    ...
    for na = 1:length(amplitudes)
        ...
        if (na == 1)
            ...
            for ch = find(sum(channelMapping, 2))'
                xfprintf(fAWG, sprintf(':VOLT%d %.15g', ch, amplitude));
            end
        end
        xfprintf(fSA, ':INIT:IMM');
        while (xquery(fSA, '*OPC?') == 0)
        ...
        mag = sscanf(xquery(fSA, 'CALC:MARK1:Y?'), '%g');
        cal(nt, na) = mag;
        result.cal = cal;
        doPlot(result, myAxes);
        if (~isempty(hMsgBox))
